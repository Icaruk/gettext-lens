...
export class StatusBarManager {
	...
	constructor() {
		this.fileItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 50);
		this.workspaceItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 49);
	}
	...
	showFile(summary: LintSummary, isTemplate: boolean): void {
		...
		if (isTemplate) {
			...
			const pct =
				summary.total > 0 ? Math.round((summary.translated / summary.total) * 100) : 0;
			parts.push(`${pct}% done`);
