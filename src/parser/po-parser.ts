...
const FIELD_KEYWORD =
  /^#~\s*(msg(?:id|str|ctxt|id_plural)(?:\[\d+\])?)\s+"(.*)"$/;
const FIELD_KEYWORD_EMPTY =
  /^#~\s*(msg(?:id|str|ctxt|id_plural)(?:\[\d+\])?)\s*$/;
const PLAIN_KEYWORD =
  /^(msg(?:id|str|ctxt|id_plural)(?:\[\d+\])?)\s+"(.*)"$/;
const PLAIN_KEYWORD_EMPTY =
  /^(msg(?:id|str|ctxt|id_plural)(?:\[\d+\])?)\s*$/;
...
function collectContinuation(
  ...
  return { text: unescape(parts.join("")), spanLines };
}
