...
import { invalidate, clearAll } from "./services/parse-cache.js";
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "gettextLens.fix.stripFuzzy",
      (uri: string, line: number) => fixes.stripFuzzy(uri, line),
    ),
    vscode.commands.registerCommand(
      "gettextLens.fix.copySource",
      (uri: string, line: number) => fixes.copySourceToTranslation(uri, line),
    ),
    vscode.commands.registerCommand(
      "gettextLens.fix.insertTags",
      (uri: string, line: number, tags: unknown) =>
        fixes.insertMissingTags(uri, line, tags as Parameters<typeof fixes.insertMissingTags>[2]),
    ),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "gettextLens.scanWorkspace",
      async () => {
        const result = await scanWorkspace();
        if (!result) {
          vscode.window.showInformationMessage("No .po or .pot files found in the workspace.");
          return;
        }
