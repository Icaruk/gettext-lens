...
import { lintDocument, type IssueData } from "../src/services/linter.js";
...
function countByKind(
  ...
  return diagnostics.filter(
    (d) => (d as { data?: IssueData }).data?.kind === kind,
  ).length;
}
...
describe("linter — edge cases", () => {
  ...
  it("skips obsolete entries entirely", () => {
    ...
    const obsoleteMessages = diagnostics.filter((d) =>
      d.message.includes("Receta antigua"),
    );
